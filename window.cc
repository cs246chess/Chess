...
void Xwindow::drawString(int x, int y, string msg, int colour) {
  XFontStruct * f = XLoadQueryFont(d, "6x13");
	...
	// delete f; // causes memory leak
	/* 
	 * Nomair: March, 2020
	 * Will call XFreeFont to deallocate f instead of using
	 * delete even though it uses free and it is considered 
	 * poor practice to mix delete and free.
	 * If issues are reported, the delete line above can
	 * be uncommented and the XFreeFont call below can be
	 * commented.
	 */
	XFreeFont(d,f);
}
